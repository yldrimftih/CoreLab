function [sysParam,txParam,payload] = OFDMSetParamsSDR(OFDMParam,dataParam)
...
sysParam.pilotIdx             = ((sysParam.FFTLen-sysParam.usedSubCarr)/2) + (1:sysParam.pilotSpacing:sysParam.usedSubCarr).';
...
if any(sysParam.pilotIdx == dcIdx)
    sysParam.pilotIdx(floor(length(sysParam.pilotIdx)/2)+1:end) = 1 + sysParam.pilotIdx(floor(length(sysParam.pilotIdx)/2)+1:end);
end
